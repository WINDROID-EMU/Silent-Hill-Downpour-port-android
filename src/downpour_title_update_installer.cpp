...
// xboxunity.net hosts the official Silent Hill: Downpour Title Update 1
// container under tuid=22574 (Konami title 4B4E0823, MediaID 7D387D17, base
// version 00000003 → TU1, container ~1.68 MB). The Microsoft Xbox Live
// download endpoint for the Xbox 360 was retired in June 2024, so this is
// the canonical community mirror. Users on a network where xboxunity is
// unreachable can override this cvar or pick the file manually.
REXCVAR_DEFINE_STRING(downpour_title_update_url,
                      "https://www.xboxunity.net/Resources/Lib/TitleUpdate.php?tuid=22574",
                      "Downpour",
                      "Download URL for the Silent Hill: Downpour Title Update 1 package (TU1). "
                      "Set to empty to hide the in-app download button and force file selection.");

namespace downpour {

namespace {

// The recompilation is generated against exactly these payloads (Silent Hill:
// Downpour title 4B4E0823, base 1.0 -> TU1, media ID 7D387D17).
...
// Downpour TU1 ships a single payload: the patched executable overlay. No
// EAWebkit / web-resource overlay (that pattern is EA-specific and present in
// Skate 3). The SHA-256 here was computed from the STFS LIVE container the
// user dumped on 2026-06-21; if Konami released regional variants with
// different .xexp hashes we'll need to grow this whitelist.
constexpr std::array<TitleUpdatePayload, 1> kPayloads = {{
    {"default.xexp", "default.xexp", 1652736,
     "8732e3266301347b4494f2ab885c0ddf6a1ab5d03403a950263f90eeb8425122"},
}};

// Size of the known TU1 STFS container; used as the progress estimate when
// the HTTP response carries no Content-Length, and as a sanity cap for
// downloads.
constexpr uint64_t kContainerSize = 1720320;
...
bool ReadWholeFile(const std::filesystem::path& path, std::vector<uint8_t>& out,
  ...
  if (size > kMaxPackageSize) {
    error = "The selected file is too large to be a Silent Hill: Downpour title update.";
    return false;
  }
...
bool StagePayload(const TitleUpdatePayload& payload, const std::vector<uint8_t>& data,
  ...
  REXLOG_INFO("Staged Silent Hill: Downpour title update payload {}", target.string());
  return true;
}

// ----------------------------------------------------------------------------
// Minimal read-only STFS (CON/LIVE/PIRS) package reader, enough to pull the
// xexp payload out of a title update package.
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// Download (WinHTTP on Windows, curl fallback elsewhere)
// ----------------------------------------------------------------------------
...
bool DownloadToFile(const std::string& url, const std::filesystem::path& destination,
  ...
  HINTERNET session = WinHttpOpen(L"downpour-recomp/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
  ...
  // IPv6 Happy Eyeballs to avoid 15s waits on hosts with dead AAAA records.
...
bool DownloadToFile(const std::string& url, const std::filesystem::path& destination,
  ...
  const std::string command = "curl -fsSL --connect-timeout 30 --max-time 600 -o '" +
                              destination_str + "' '" + url + "'";
  REXLOG_INFO("Downloading Silent Hill: Downpour title update via curl");
...
std::filesystem::path PickTitleUpdateFile() {
  ...
  ofn.lpstrTitle = L"Select the Silent Hill: Downpour Title Update 1 package";
...
std::filesystem::path PickTitleUpdateFile() {
  GtkWidget* dialog = gtk_file_chooser_dialog_new(
      "Select the Silent Hill: Downpour Title Update 1 package", nullptr,
...
bool DownloadAndStageTitleUpdate(const std::filesystem::path& game_root,
  ...
  if (url.empty()) {
    error = "No download URL is configured for the Silent Hill: Downpour title update. "
            "Use 'Select file...' to point at a TU1 STFS container or raw default.xexp.";
    return false;
  }
  REXLOG_INFO("Downloading Silent Hill: Downpour title update from {}", url);
...
bool StageTitleUpdateFromFile(const std::filesystem::path& source,
    ...
    for (const auto& payload : kPayloads) {
      ...
      if (it == entries.end()) {
        error = "The package does not contain " + std::string(payload.container_path) +
                "; it is not the Silent Hill: Downpour title update.";
        return false;
      }
      ...
      if (file_data.size() != payload.size ||
        ...
        error = "The package contains a different version of " +
                ...
                "; this build requires Title Update 1.";
        return false;
      }
  ...
  // Not an STFS container; accept a raw xexp payload that matches the
  // expected SHA-256 exactly.
  if (const auto* payload = MatchPayload(data)) {
    return StagePayload(*payload, data, game_root, error);
  }

  error =
      "The selected file is neither the Silent Hill: Downpour title update package nor its "
      "default.xexp payload.";
  return false;
}
...
void ShowTitleUpdateInstallWizard(rex::ui::ImGuiDrawer* drawer, rex::PathConfig runtime_paths,
  ...
  options.title = "Silent Hill: Downpour Title Update 1";
  options.intro =
      "This build of Silent Hill: Downpour requires Title Update 1, a free update originally "
      "published on Xbox Live. It is not part of the game disc.";
  options.target_directory = game_root.string();
  options.initial_status =
      "Download Title Update 1 automatically, or select a TU1 STFS container / "
      "raw default.xexp file you already have.";
  // Hide the Download button only if a user has explicitly cleared the cvar.
  const std::string configured_url = REXCVAR_GET(downpour_title_update_url);
  if (!configured_url.empty()) {
    ...
    options.fetch_working_status = "Downloading Title Update 1...";
  }
  options.pick_button_label = "Select file...";
  options.install_working_status = "Installing the title update...";
  options.done_status = "Title Update 1 installed.";
...
bool RunTitleUpdateInstallWizardBlocking(rex::ui::WindowedAppContext& app_context,
  ...
  if (const char* automated_tu = std::getenv("DOWNPOUR_INSTALL_TU");
      automated_tu != nullptr && *automated_tu != '\0') {
    std::string error;
    bool ok = false;
    REXLOG_INFO("Installing Silent Hill: Downpour title update from DOWNPOUR_INSTALL_TU={}",
                automated_tu);
  ...
  REXLOG_INFO("Entering Silent Hill: Downpour title update installer pump");
